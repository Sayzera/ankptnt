let ApplicationFile = {
  ...
  pasifAciklama : `Markanız 'Hızlı Gözlem' listesinde değildir. 'Hızlı Gözlem' listesine eklemek için tıklayınız. Bu işlem sizin daha hızlı gözlem yapmanızı sağlar.  `,
  dataTable: function (tableId) {
    ...
    $(tableId).DataTable({
      ...
      ajax: {
        url: scriptData.getTrademarkUrl + "?type=" + scriptData.type + "&gozlem_durumu=" + scriptData.gozlem_durumu + "&bulten=" + scriptData.bulten,
        type: "GET",
      ...
      drawCallback: function (settings) {
        ...
        if(scriptData.type != 3 && scriptData.type != 4)  {
          $('.marka-result').html(`<span style="text-decoration:underline">${scriptData.bulten}</span>. bülten için 
          ${scriptData.type ==0 
          ? 'gözlemi yapılamamış '
          : scriptData.type == 2
          ? 'gözlemi yapılmış' 
          ...
          (${(json?.data?.length || '0')} adet marka bulundu)`)

        } else {
          $('.marka-result').text('('+ (json?.data?.length || '0') + ' adet marka bulundu )')
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
            ...
            return `<div id="marka-aktif-pasif-div-${row.col_id}" style="width:100% !important;" class="d-flex justify-content-between gap-2 aligin-items-center ana-div-icon">
            <div class="d-none aktif-pasif-loading">Bekleyiniz...</div>
            <span 
         
            id="marka-aktif-pasif-span-${row.col_id}"  onclick="ApplicationFile.markalarinGozlemDurumuAktifPasif(${row.col_id})"  class="
            ${row.gozlem_durumu == null ? 'text-success' : row.gozlem_durumu == 1 ? 'text-danger' : 'text-success'} 
            " style="cursor:pointer">
            ${row.gozlem_durumu == null ? 'Hızlı Gözleme Ekle' : row.gozlem_durumu == 1 ? 'Hızlı Gözlemden Kaldır' : 'Hızlı Gözleme Ekle'} 
            </span>

            <i class="fas fa-info-circle alt-icon-info" id="marka-aktif-pasif-icon-${row.col_id}" style="font-size:18px; color:#5bc0de"    data-tippy-content="  ${row.gozlem_durumu == null ? ApplicationFile.pasifAciklama : row.gozlem_durumu == 1 ? ApplicationFile.aktifAciklama : ApplicationFile.pasifAciklama} "></i>
            </div>`;
          }
        }
      ],
          ...
          .each(function (colIdx) {
            ...
            if (title.trim() === "Aktif/Pasif") {
              ...
              <select class="form-select form-select-sm" aria-label=".form-select-sm example" id="aktif-pasif-select-box">
                ...
                <option value="3">Aktif Markalar</option>
                <option value="4">Pasif Markalar</option>
              </select>
              `);
            } else {
